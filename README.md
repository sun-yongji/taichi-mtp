...
现有 MTP（如 DeepSeek-V3）使用固定预测深度。TaiChi-MTP 以 **六群元素承乘比应关系** 构造自适应深度调度——归一化熵 < 0.90 浅槽位单步预测，0.90~0.97 中槽位逐步细化，> 0.97 深槽位全六步推演。六个预测 head 通过 C6 循环群耦合矩阵连接：c_{ij} = |∑ω^{k(i-j)}|/6。

**关键现象**：湍流模式下 head 耦合差异达 100:1（标准 MTP 仅 10:1），意味着六群元素调度能从高熵输入提取 10 倍于标准方法的结构信息。
...
TaiChi-MTP 是TaiChi Matrix 六模块工具链的 M2 站：
